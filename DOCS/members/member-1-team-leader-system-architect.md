...
- Bao ve `main`: chi approve/promotion tu `testing_branch` sau khi M6 da ghi `Pass` cho candidate tich hop; tu choi feature branch merge truc tiep vao `main`.

## Core Assignments

| Due | Task | Dependency | Required evidence |
| --- | --- | --- | --- |
| `2026-05-27` | Confirm recovery scope, canonical auth/API/DB, retained extension va `testing_branch` -> test -> `main` rule | Recovery report | Approved decision entries and team branch notice |
...
- `main` chi chua candidate da duoc M6 verify `Pass` tren `testing_branch` va duoc M1 approve.
