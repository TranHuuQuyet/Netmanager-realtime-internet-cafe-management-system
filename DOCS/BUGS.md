...
| `B-002` | Critical | TCP listener/dispatcher round-trip awaits final gate verification | `R1-N01/R1-N02` candidate uses `TcpJsonLineServer` + `PacketDispatcher`; `dotnet run --project Code/NetworkSmokeTest/NetworkSmoke.csproj --no-restore` verifies authenticated `LOGIN` success/failure and controlled disconnects for malformed JSON, an unknown packet type and unopened `STATUS`. A fresh valid login succeeds after each rejected socket. | M2 + M6 | `R1/G1` | `R1-N02` evidence submitted `2026-06-02`; M6 verification pending |
