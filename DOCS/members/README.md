...
- Moi member tao feature/fix branch tu `testing_branch` va nop PR/merge vao `testing_branch`; khong merge feature truc tiep vao `main`.
- `testing_branch` la nhanh integration de M6 test; chi candidate duoc M6 ghi `Pass` va M1 approve moi duoc merge/promote vao `main`.
...
Each handoff includes interface/result shape, evidence or blocker, consumer, due date and affected gate. M6 verifies pass status on `testing_branch`; M1 approves gate transitions, extension opening and promotion from `testing_branch` to `main`.
