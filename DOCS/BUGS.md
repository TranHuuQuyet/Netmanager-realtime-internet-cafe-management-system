...
| `B-008` | High | Client-sent `STATUS` route is missing | Closed by `2026-06-15` local verification on `testing_branch` commit `222c68c`: `ClientMainForm.SendResumeStatusAsync()` sends authenticated `STATUS`, `PacketDispatcher.DispatchStatusAsync()` accepts it and returns typed `ACK` with `ackFor=STATUS` / `status=Accepted`, and `TcpJsonLineServer.StatusEmitted` updates the dashboard bridge. Evidence: `DOCS/VERIFICATION/R3_B008_STATUS_VERIFICATION.md`; `dotnet run --project Code\NetworkSmokeTest\NetworkSmoke.csproj --no-restore` prints `PASS: authenticated STATUS route returns Accepted ACK`. | M2 + M4 + M6 | `R2/G2`, `R3-B01/G3-00` | Verified Pass (local audit - 2026-06-15) |
